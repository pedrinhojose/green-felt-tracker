    ...
    if (this.useSupabase) {
      try {
        ...
        return data.map(season => ({
          ...
          expectedEndDate: (season as any).expected_end_date ? new Date((season as any).expected_end_date) : undefined,
    ...
    if (this.useSupabase) {
      try {
        ...
        return {
          ...
          expectedEndDate: (data as any).expected_end_date ? new Date((data as any).expected_end_date) : undefined,
    ...
    if (this.useSupabase) {
      try {
        ...
        const supabaseSeason = {
          ...
          expected_end_date: season.expectedEndDate ? season.expectedEndDate.toISOString().split('T')[0] : null,
