...
export function usePlayerStats(seasonId?: string) {
  ...
  useEffect(() => {
    const calculateStats = async () => {
      ...
      if (seasonId && seasonId !== activeSeason?.id) {
        try {
          // Buscar temporada específica
          targetSeason = await pokerDB.getSeason(seasonId);
          if (!targetSeason) {
          ...
          targetGames = await pokerDB.getGames(seasonId);
          
          // Buscar rankings da temporada específica
          targetRankings = await pokerDB.getRankings(seasonId);
          
          console.log("Loaded data for season:", seasonId);
          console.log("- Games:", targetGames.length);
          console.log("- Rankings:", targetRankings.length);
        } catch (error) {
    ...
    calculateStats();
  }, [games, activeSeason, players, rankings, seasonId]); // Adicionar seasonId como dependência
