...
export function TimerCenterDisplay({ 
  ...
  return (
    ...
    >
      {/* Container flutuante com efeito de vidro */}
      <div 
      ...
      >
        {/* Backdrop blur com bordas de vidro */}
        <div 
          className={`relative backdrop-blur-md bg-white/5 border border-white/20 rounded-3xl ${isMobile ? 'p-6' : 'p-12'} shadow-2xl`}
          style={{
        ...
        >
          {/* Reflexo no topo */}
          <div 
            className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-1 bg-gradient-to-r from-transparent via-white/30 to-transparent rounded-full"
            style={{ filter: 'blur(1px)' }}
          ></div>
          
          {/* Timer principal com efeito 3D */}
          <div 
            className={`${isMobile ? 'text-[60px]' : 'text-[120px]'} font-bold ${getTimeColor()} transition-all duration-300 font-mono leading-none text-center relative`}
            style={{
          ...
          {/* Label "Tempo" com efeito de profundidade */}
          <div 
            className={`text-poker-gold ${isMobile ? 'text-lg' : 'text-3xl'} font-normal ${isMobile ? 'mt-1' : 'mt-4'} text-center relative`}
            style={{
